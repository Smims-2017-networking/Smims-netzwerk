...
public class ClientGUI extends JFrame {
	...
	private final Color[] Colors = { Color.BLUE, Color.RED, Color.GREEN, Color.YELLOW, Color.PINK, Color.ORANGE,
	...
	public void updateBoard(Board pBoard) {
		...
		System.out.println("myBoardDescriptor.getBoardSize(): " + myBoardDescriptor.getBoardSize()
				+ " ;myBoardDescriptor.getHouseSize(): " + myBoardDescriptor.getHouseSize()
				+ "; myBoard.getBoardSectionSize(): " + myBoard.getBoardSectionSize());
		this.repaint();
		drawBoard();
		if (gameCharacters.isEmpty()) {
			placeCharactersOnBoard(pBoard.getAllCharacters());
		}
		System.out.println("call updateCharactersOn");
	...
	private void placeCharactersOnBoard(Collection<Character> pCharacters) {
		...
		System.out.print("pCharacters.size() = " + pCharacters.size());
	 ...
	 * 
	 * @param pPunkt
	 *            Punkt ist der MittelPunkt des Feldes
	 */
	 ...
	 * 
	 * @param pHouse
	 *            von welchem Spieler das Haus
	 * @param pHouseFieldNumber
	 *            das wievielete haus feld
	 * @return
	 */
	public Punkt getPunktByHouseAndNumber(int pHouse, int pHouseFieldNumber) {
		...
		{
			return new Punkt(FieldStartX + distanceBetweenFields * (myBoardDescriptor.getBoardSize() - 1),
					FieldStartY + distanceBetweenFields * (pHouseFieldNumber + 1));
		}
		return new Punkt(
				FieldStartX - distanceBetweenFields + distanceBetweenFields * myBoard.getBoardSectionSize() * pHouse,
				FieldStartY + distanceBetweenFields * (pHouseFieldNumber + 1));
	}

	public Punkt getPunktByBaseAndNumber(int pBase, int pBaseFieldNumber) {
		return new Punkt(FieldStartX + distanceBetweenFields * myBoard.getBoardSectionSize() * pBase,
				FieldStartY - distanceBetweenFields * (pBaseFieldNumber + 2));
	}
	...
	public void selectCharacter(Position pPosition) {
		...
		if (pPosition.isInBase()) {
			...
			myClient.send(Protokoll.CS_MoveCharacter + Protokoll.Splitter + Protokoll.House + Protokoll.Splitter
					+ pPosition.getHouseFieldNumber().get());
		} else if (pPosition.isOnField() && !pPosition.isAtStartingPosition()) {
			myClient.send(Protokoll.CS_MoveCharacter + Protokoll.Splitter + Protokoll.Board + Protokoll.Splitter
					+ pPosition.getFieldNumber().get());
		} else if (pPosition.isOnField() && pPosition.isAtStartingPosition()) {
			myClient.send(
